...
# @app.route('/start_screenshot_app', methods=['POST'])
# def start_screenshot_app():
#     try:
#         subprocess.Popen(['python', 'screenshot_app.py'])  # Non-blocking
#         return jsonify({'status': 'success', 'message': 'Screenshot app started.'})
...
def save_selected_checks():
    ...
    for check_name in selected:
        if user_elem.find(f"./check[name='{check_name}']") is None and check_name in defs:
            check_elem = ET.SubElement(user_elem, "check")
            ET.SubElement(check_elem, "name").text = check_name
            # Don't store how_to_perform here — rely on check_definitions.xml instead
            ET.SubElement(check_elem, "expected_outcome").text = defs[check_name]['expected_outcome']
...
def view_screenshots():
    subprocess.Popen(["python", "local_screenshot.py"])  # Name of your PyQt script
    return """
        <script>
            alert("Screenshot tool launched successfully!");
...
def save_table():
    ...
    filepath = os.path.join("saved_tables", "check_verified_table.xml")
    tree = ET.ElementTree(root)
...
def get_check_definitions():
    ...
    for check in root.findall('check'):
        name = check.find('name').text
        how_elem = check.find('how_to_perform')
        description = how_elem.find('description').text if how_elem.find('description') is not None else ''
        text = how_elem.find('text').text if how_elem.find('text') is not None else how_elem.text
        link = how_elem.find('link').text if how_elem.find('link') is not None else ''
        outcome = check.find('expected_outcome').text
        checks[name] = {
            'how_to_description': description,
            'how_to_text': text,
            'how_to_link': link,
    ...
    if user_elem is not None:
        for check in user_elem.findall('check'):
            ...
            if name in defs:
                ...
                selected.append({
                    'name': name,
                    'how_to_perform': f"{entry['how_to_description']} <a href='{entry['how_to_link']}' target='_blank'>{entry['how_to_text']}</a>",
